...
function keepSigninFieldsOnly(formData) {
  Object.keys(ENTRIES_NAMES).forEach((name) => {
    if (ENTRIES_NAMES.email && name !== ENTRIES_NAMES.password) {
...
async function signin(formData) {
  ...
  formData.set('login', formData.get(ENTRIES_NAMES.email));
  formData.delete(ENTRIES_NAMES.email);
  const { data, error } = await postSignin(formData);
  if (error) {
    throw error.message
